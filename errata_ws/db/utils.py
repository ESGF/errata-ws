...
from errata_ws.db import session
...
class _BaseEntity(object):
    ...
    """
    # Default columns appearing upon all tables.
    id = Column(Integer, primary_key=True)
    row_create_date = Column(DateTime, nullable=False, default=dt.datetime.utcnow)
    row_update_date = Column(DateTime, onupdate=dt.datetime.utcnow)
...
def text_filter(qry, field, key):
    ...
    """
    return qry.filter(sqlalchemy.func.upper(field) == str(key).strip().upper())
